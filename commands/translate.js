...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('translate')
    .setDescription('Übersetze Text in eine andere Sprache')
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      ...
      try {
        ...
        await i.update({
          content: '❌ Fehler bei der Übersetzung. Bitte versuche es später erneut.',
          components: []
        });
      }
    });
    ...
    collector.on('end', collected => {
      if (collected.size === 0) {
        interaction.editReply({
          content: '⏱️ Zeit abgelaufen – keine Sprache gewählt.',
          components: []
        }).catch(() => {});
      }
    });
